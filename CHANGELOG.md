...
- UNCOMMITTED 2026-03-08 Jeff Tratt — FIX: `#name` submenu accelerator conflict
  - The “type of an object in inventory” entry no longer uses 'n' as a group
...
- UNCOMMITTED 2026-03-08 Jeff Tratt — UI: inventory menus triggered by "Put in what?"/"Take out what?" now show the normal inventory header, and when objects are grouped by class the category heading includes the combined weight for that class (e.g. "WANDS 35×7u").  This gives quicker insight into the cost/weight of each category while looting or stashing.
  - FIX: avoid drawing the inventory header twice in curses menus. Earlier code added the heading both to the prompt and as a nonselectable menu entry; the extra line is now removed so users see a single header.
...
- UNCOMMITTED 2026-03-08 Jeff Tratt — FIX: pet interpose threat ranking
  ...
  - Follow-up fix: when urgent interpose selected the pet's current square as the best guard square, the code short-circuited into the movement/hold-position path and skipped the normal adjacent melee attack check. Pets now keep the guard-square preference without suppressing a legal attack against an adjacent hostile.
