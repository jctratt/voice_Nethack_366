...
register struct monst *mtmp;
{
    ...
    case MS_MEW:
        if (mtmp->mtame) {
            if (mtmp->mconf || mtmp->mflee || mtmp->mtrapped
                || mtmp->mtame < 5)
                pline_msg = "yowls troublesomely.";
            else if (moves > EDOG(mtmp)->hungrytime)
                pline_msg = "meows hungrily.";
            else if (EDOG(mtmp)->hungrytime > moves + 1000)
                pline_msg = "purrs contentedly.";
            else
                pline_msg = "mews.";
            break;
        }
        /*FALLTHRU*/
    case MS_GROWL:
        pline_msg = mtmp->mpeaceful ? "snarls troublesomely." : "growls troublesomely!";
        break;
    case MS_ROAR:
        pline_msg = mtmp->mpeaceful ? "snarls troublesomely." : "roars troublesomely!";
        break;
    case MS_SQEEK:
        pline_msg = "squeaks.";
        break;
    ...
    case MS_NEIGH:
        if (mtmp->mtame < 5)
            pline_msg = "neighs troublesomely.";
        else if (moves > EDOG(mtmp)->hungrytime)
            pline_msg = "whinnies hungrily.";
        else
