...
static struct Bool_Opt {
    ...
    { "petai_vanilla", &iflags.vanilla_pet_behavior, FALSE, SET_IN_GAME },
    ...
    { "shockwave", &iflags.shockwave, TRUE, SET_IN_GAME },
    { "shockwave_sparkle", &iflags.shockwave_sparkle, TRUE, SET_IN_GAME },
...
static struct Comp_Opt {
    ...
    { "name", "your character's name (e.g., name:Merlin-W)", PL_NSIZ,
      DISP_IN_GAME },
    { "mouse_support", "game receives click info from mouse", 0,
      SET_IN_GAME },
    ...
    { "statushilites",
      ...
      "0=no status highlighting, N=show highlights for N turns", 20,
      SET_IN_GAME
#else
    ...
    { "statuslines",
#ifdef CURSES_GRAPHICS
      "2 or 3 lines for horizontal (bottom or top) status display", 20,
      SET_IN_GAME
#else
      "2 or 3 lines for status display", 20, SET_IN_FILE
#endif
    }, /*WC2*/
    ...
    { "whatis_coord", "show coordinates when auto-describing cursor position",
      1, SET_IN_GAME },
    { "whatis_filter",
      "filter coordinate locations when targeting next or previous", 1,
      SET_IN_GAME },
...
initoptions_init()
{
    ...
    flags.voice_engine[0] = '\0';  // Set to empty string by default
...
/* allow option to be silently ignored by non-tty ports */
    ...
    if (match_optname(opts, fullname, 11, TRUE)) {
        ...
        if (negated) {
            ...
            if (result < 0) {
                config_error_add("Could not compile VOICE_FORCE regex: '%s'",
                                 op);
                return FALSE;
            } else if (!initial && !tfrom_file) {
                ; // config_error_add("Successfully compiled VOICE_FORCE:
                  // '%s'", op); /* Optional debug */
            }
        } else {
            return FALSE;
    ...
    if (match_optname(opts, fullname, 12, TRUE)) {
        ...
        if ((negated && op == empty_optstr)
            ...
            if (negated) {
                ...
                strncpy(flags.voice_command, op,
                        sizeof(flags.voice_command) - 1);
                flags.voice_command[sizeof(flags.voice_command) - 1] =
            ...
            flags.voice_command[0] =
                '\0'; // Set to empty if no value provided
        }
        return retval;
    }

    fullname = "voice_engine";
    if (match_optname(opts, fullname, 12, TRUE)) {
        if (duplicate)
            complain_about_duplicate(opts, 1);
        op = string_for_opt(opts, negated);
        if ((negated && op == empty_optstr)
            || (!negated && op != empty_optstr)) {
            if (negated) {
                flags.voice_engine[0] = '\0'; // Reset to empty if negated
            } else {
                strncpy(flags.voice_engine, op,
                        sizeof(flags.voice_engine) - 1);
                flags.voice_engine[sizeof(flags.voice_engine) - 1] =
                    '\0'; // Ensure null termination
            }
        } else if (negated) {
            bad_negation(fullname, TRUE);
            ...
            flags.voice_engine[0] = '\0'; // Set to empty if no value provided
        }
        return retval;
    }
#endif /* VOICE_ENABLED */
        ...
        if (negated) {
            ...
            for (;;) {
                ...
                if (i == SIZE(paranoia)) {
                    ...
                    config_error_add("Unknown %s parameter '%s'", fullname,
                                     op);
                    return FALSE;
                }
...
doset() /* changing options via menu by Per Liboriussen */
{
    ...
    add_menu(
        tmpwin, NO_GLYPH, &any, 0, 0, iflags.menu_headings,
    ...
    if (!strcmp("menustyle", optname)) {
        ...
        for (i = 0; i < NUM_DISCLOSURE_OPTIONS; i++) {
            if (disc_cat[i]) {
                ...
                Sprintf(buf,
                        "Disclosure options for %s:", disclosure_names[i]);
                tmpwin = create_nhwindow(NHW_MENU);
                start_menu(tmpwin);
                any = zeroany;
                ...
                if (*disclosure_names[i] == 'v') {
                    ...
                    add_menu(tmpwin, NO_GLYPH, &any, 0, any.a_char, ATR_NONE,
                             "Prompt, with default answer of \"Ask\" to "
                             "request sort menu",
                             (c == any.a_char) ? MENU_SELECTED
                                               : MENU_UNSELECTED);
                }
...
#undef COL80ARG
    ...
    ape_again:
        ...
        if (res && symset_list) {
            ...
            Sprintf(buf,
                    "Select %ssymbol set:", rogueflag ? "rogue level " : "");
            end_menu(tmpwin, buf);
...
#undef MOUSEFIX2
    ...
    } else if (!strcmp(optname, "voice_engine")) {
        Sprintf(buf, "%s", flags.voice_engine);
...
dotogglepickup()
{
    ...
    if (flags.pickup) {
        ...
        Sprintf(buf, "ON, for %s objects%s", ocl[0] ? ocl : "all",
                (apelist) ? ((count_apes() == 1) ? ", with one exception"
...
add_voice_exception(const char *pattern)
{
    struct voice_exception *newve =
        (struct voice_exception *) alloc(sizeof *newve);
...
check_voice_exceptions(const char *text)
{
...
add_voice_force(const char *pattern)
{
    ...
    if (sep) {
        ...
        if (!match_pattern) {
            config_error_add(
                "Memory allocation failed for VOICE_FORCE pattern");
            free(newvf);
            return -1;
        }
        ...
        if (!speak_text) {
            config_error_add(
                "Memory allocation failed for VOICE_FORCE speak_text");
            free(match_pattern);
            free(newvf);
            return -1;
        }
    ...
    if (reti) {
        ...
        config_error_add("Could not compile VOICE_FORCE regex '%s': %s",
                         match_pattern, errbuf);
        free(match_pattern);
        free(speak_text);
        free(newvf);
        regfree(&regex);
...
struct voice_force *
check_voice_forces(const char *text)
{
